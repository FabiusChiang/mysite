...
echo ${hostName}
echo ${userName}
docker run -p ${port}:80 --name ${wordPressContainerName} -e WORDPRESS_DB_HOST=${hostName} -e WORDPRESS_DB_PASSWORD="${specialPass}" -e WORDPRESS_DB_NAME=wordpress_${appEnv} -e WORDPRESS_DB_USER="${userName}" -e WP_HOME="https://blog-qa.fabius.tk" -e WP_SITEURL="HTTPS://blog-qa.fabius.tk" -d wordpress:4.9.1-php5.6-apache
